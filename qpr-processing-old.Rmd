...
sessional_prov_mapping <- as.data.frame(read_excel("master_amb_sessional_provider_mapping.xlsx", col_types = "text"))
...
scheduling_data_raw <- scheduling_data_raw %>%
  mutate(unique_id = paste0(Department, Provider))

sessional_prov_mapping <- sessional_prov_mapping %>%
  mutate(unique_id = paste0(Department, Provider))


scheduling_data_raw <- left_join(scheduling_data_raw, sessional_prov_mapping, by = c("unique_id"))
...
appt_type_mapping <- as.data.frame(read_excel("master_amb_appt_type_mapping.xlsx", col_types = "text"))
...
scheduling_data_raw <- scheduling_data_raw %>%
  mutate(unique_id = paste0(Campus.Specialty.Group, Appt.Type))

appt_type_mapping <- appt_type_mapping %>%
  mutate(unique_id = paste0(Campus.Specialty.Group, `Appointment type`))
...
scheduling_data_raw <- scheduling_data_raw %>%
  mutate(DepartmentId = as.character(DepartmentId))
...
referrals <- left_join(referrals, sessional_prov_mapping, 
                       by = c("ReceivedByDept" =  "Department",
...
# ## MSHS New Patient Wait Time Horizon ===============================================
# waitTime_overview_provider <- scheduling_data_raw %>%
...
#   filter(Appt.Made.DateYear >= quarter_start & Appt.Made.DateYear < quarter_end) 
# 
# waitTime_overview_provider <- waitTime_overview_provider %>%  
#   group_by(Campus.Specialty.Group, Campus.Specialty.SubGroup, Site, Department, Provider, Appt.Made.YearQtr, New.PT2) %>%
#   mutate(med_wait = median(Wait.Time, na.rm = TRUE)) %>%
#   group_by(Campus.Specialty.Group, Campus.Specialty.SubGroup, Site, Department, Provider, Appt.Made.YearQtr, New.PT2, Visit.Method, med_wait, group) %>%
#   summarise(total = n())
# 
# require(openxlsx)
# write.xlsx(waitTime_overview_provider, "Cardiology Provider Wait Time.xlsx")
