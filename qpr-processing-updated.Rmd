...
# neurosurg_scheduling <- scheduling_data_raw_tbl %>%
#   filter(Managed == "Department") %>%
#   filter(Campus.Specialty.Group == "Neurosurgery") %>%
#   filter(Appt.Made.Year >= 2022) %>%
#   collect()
#   
# require(openxlsx)
# write.xlsx(neurosurg_scheduling, "Neurosurgery Appts Scheduled_03.15.24.xlsx")

```

```{r Manual Import Wait Time Data, echo = FALSE, warning = FALSE, message = FALSE}

# waitTime_dist_specialty_ytd <- readRDS("waitTime_dist_specialty_ytd.rds")
# waitTime_dist_site_ytd <- readRDS("waitTime_dist_site_ytd.rds")
# waitTime_mshs_ytd <- readRDS("waitTime_mshs_ytd.rds") 
# waitTime_site_ytd <- readRDS("waitTime_site_ytd.rds") 
# waitTime_prov_ytd <- readRDS("waitTime_prov_ytd.rds") 
# waitTime_dist_specialty_group_ytd <- readRDS("waitTime_dist_specialty_group_ytd.rds") 
# waitTime_dist_site_group_ytd <- readRDS("waitTime_dist_site_group_ytd.rds") 
# waitTime_mshs_group_ytd <- readRDS("waitTime_mshs_group_ytd.rds") 
# waitTime_site_group_ytd <- readRDS("waitTime_site_group_ytd.rds") 
# waitTime_prov_group_ytd <- readRDS("waitTime_prov_group_ytd.rds") 
# 
# 
# waitTime_dist_specialty_office_ytd <- readRDS("waitTime_dist_specialty_office_ytd.rds")
# waitTime_dist_site_office_ytd <- readRDS("waitTime_dist_site_office_ytd.rds")
# waitTime_mshs_office_ytd <- readRDS("waitTime_mshs_office_ytd.rds")
# waitTime_site_office_ytd <- readRDS("waitTime_site_office_ytd.rds")
# waitTime_prov_office_ytd <- readRDS("waitTime_prov_office_ytd.rds")
# waitTime_dist_specialty_group_office_ytd <- readRDS("waitTime_dist_specialty_group_office_ytd.rds")
# waitTime_dist_site_group_office_ytd <- readRDS("waitTime_dist_site_group_office_ytd.rds")
# waitTime_mshs_group_office_ytd <- readRDS("waitTime_mshs_group_office_ytd.rds")
# waitTime_site_group_office_ytd <- readRDS("waitTime_site_group_office_ytd.rds")
# waitTime_prov_group_office_ytd <- readRDS("waitTime_prov_group_office_ytd.rds")
...
waitTime_dist_site_group_ytd <- waitTime_ytd %>%
  ...
  mutate(med_wait = median(Wait.Time, na.rm = TRUE)) %>%
  group_by(Campus.Specialty.Group, Appt.Made.Year, New.PT2, Visit.Method, group) %>%
  mutate(med_wait_group = median(Wait.Time, na.rm = TRUE)) %>%
  group_by(Campus.Specialty.Group, Site, Appt.Made.Year, New.PT2, Visit.Method, med_wait, med_wait_group, group) %>%
  summarise(total = n()) %>%
  collect()
...
# medicine_2022 <- scheduling_data_raw_tbl %>%
...
#   filter(Campus.Specialty.Group == "Medicine") %>%
#   collect()
# 
# medicine_2023 <- scheduling_data_raw_tbl %>%
...
# 
# require(openxlsx)
# write.xlsx(medicine_2022, "Medicine Arrived Data_2022.xlsx")
# write.xlsx(medicine_2023, "Medicine Arrived Data_2023.xlsx")
# 
# write.csv(medicine_2022, "Medicine Arrived Data_2022.csv")
# write.csv(medicine_2023, "Medicine Arrived Data_2023.csv")
...
```{r Execute Monthly Volume Office Table, echo = FALSE, warning = FALSE, message = FALSE}

monthly_vol_office_table <- scheduling_data_raw_tbl %>%
  filter(APPT_DTTM >= TO_DATE(quarter_start_date,"YYYY-MM-DD HH24:MI:SS") & APPT_DTTM <= TO_DATE(quarter_end_date,"YYYY-MM-DD HH24:MI:SS")) %>%
  filter(Managed == "Department") %>%
  filter(Office == "Office") %>%
  filter(!is.na(Site)) %>%
...
# Truncate the table
dbExecute(conn1, "TRUNCATE TABLE QPR_MONTHLY_VOLUME_OFFICE")

## Write to Oracle 
batch_size <- 1000
 
index_squence <- seq(1, nrow(monthly_vol_office_table), by = batch_size)
for (i in index_squence) {
  if(i == max(index_squence)) {
    batch <- monthly_vol_office_table[i:nrow(monthly_vol_office_table), ]
  } else {
    batch <- monthly_vol_office_table[i:(i + batch_size - 1), ]
  }
  dbWriteTable(conn = conn1, name = "QPR_MONTHLY_VOLUME_OFFICE", value = batch, row.names = FALSE, append = TRUE)
  print(paste0("i is:", i))
  print(batch)
}
...
```{r Execute Volume by Zipcode Table, echo = FALSE, warning = FALSE, message = FALSE}

# Truncate the table
dbExecute(conn1, "TRUNCATE TABLE QPR_VOL_ZIPCODE")

## Write to Oracle 
batch_size <- 1000
 
index_squence <- seq(1, nrow(vol_zip_code), by = batch_size)
for (i in index_squence) {
  if(i == max(index_squence)) {
    batch <- vol_zip_code[i:nrow(vol_zip_code), ]
  } else {
    batch <- vol_zip_code[i:(i + batch_size - 1), ]
  }
  dbWriteTable(conn = conn1, name = "QPR_VOL_ZIPCODE", value = batch, row.names = FALSE, append = TRUE)
  print(paste0("i is:", i))
  print(batch)
}
...
```{r Execute Volume by Zipcode office Table, echo = FALSE, warning = FALSE, message = FALSE}

# Truncate the table
dbExecute(conn1, "TRUNCATE TABLE QPR_VOL_ZIPCODE_OFFICE")

## Write to Oracle 
batch_size <- 1000
 
index_squence <- seq(1, nrow(vol_zip_code_office), by = batch_size)
for (i in index_squence) {
  if(i == max(index_squence)) {
    batch <- vol_zip_code_office[i:nrow(vol_zip_code_office), ]
  } else {
    batch <- vol_zip_code_office[i:(i + batch_size - 1), ]
  }
  dbWriteTable(conn = conn1, name = "QPR_VOL_ZIPCODE_OFFICE", value = batch, row.names = FALSE, append = TRUE)
  print(paste0("i is:", i))
  print(batch)
}
...
# 
# require(openxlsx)
# write.xlsx(neurosurg, "Neurosurgery Referrals_2023.xlsx")
# 
# 
# 
# check <- neurosurg %>%
#   group_by(ReceivedByDept, ProcedureName) %>%
#   summarise(total = n())
...
```{r Execute Monthly Referrals Table, echo = FALSE, warning = FALSE, message = FALSE}

# # Truncate the table
# dbExecute(conn1, "TRUNCATE TABLE QPR_MONTHLY_REFERRALS")
# 
# ## Write to Oracle 
# batch_size <- 1000
#  
# index_squence <- seq(1, nrow(monthly_referral_vol), by = batch_size)
# for (i in index_squence) {
#   if(i == max(index_squence)) {
#     batch <- monthly_referral_vol[i:nrow(monthly_referral_vol), ]
#   } else {
#     batch <- monthly_referral_vol[i:(i + batch_size - 1), ]
#   }
#   dbWriteTable(conn = conn1, name = "QPR_MONTHLY_REFERRALS", value = batch, row.names = FALSE, append = TRUE)
#   print(paste0("i is:", i))
#   print(batch)
# }
