...
waitTime_site_specialty_ytd_merged <- bind_rows(waitTime_dist_specialty_ytd %>% mutate(type = "All"),
...
index_squence <- seq(1, nrow(waitTime_site_specialty_ytd_merged), by = batch_size)
for (i in index_squence) {
  if(i == max(index_squence)) {
    batch <- waitTime_site_specialty_ytd_merged[i:nrow(waitTime_site_specialty_ytd_merged), ]
  } else {
    batch <- waitTime_site_specialty_ytd_merged[i:(i + batch_size - 1), ]
  }
